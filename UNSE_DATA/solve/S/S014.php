...
$sql = "select DB_data from S014 where DB_express = '". $toYC_sibsin ."-". $woon_y ."'";
...
for ($i = 0 ; $i<$all; $i++) {
	...
	$Tb_S014aa = stripslashes($row[DB_data]);
}

if($toYC_sibsin==$toYG_sibsin && $woon_y==$woon_z){
	$woon_z=$woon_z+1;
	if($woon_z>3){$woon_z=1;}
	$woon_z="0".$woon_z;
}

$sql = "select DB_data, DB_numerical from S014 where DB_express = '". $toYG_sibsin ."-". $woon_z ."'";
$query = mysql_query($sql);
$all = mysql_affected_rows();
...
for ($i = 0 ; $i<$all; $i++) {
	mysql_data_seek($query,$i);
	$row = mysql_fetch_array($query);
	$Tb_S014bb = stripslashes($row[DB_data]);
	$Tb_S014_numerical = stripslashes($row[DB_numerical]);
}


$Tb_S014=$Tb_S014aa.$Tb_S014bb;
