...
$sql = "SELECT count(DB_num) as cnt FROM F007 WHERE DB_title_1 like '%".$dream_name."%'";
...
$sqlaa = "SELECT  DB_data, DB_title_1 from F007 WHERE DB_title_1 like '%".$dream_name."%' and DB_num <= (SELECT MIN(DB_num) FROM (SELECT DB_num FROM F007 order by DB_num desc limit ".((($Absolutepage-1)*$PageSize)+1).", 10) AS A) order by DB_num desc limit ".((($Absolutepage-1)*$PageSize)+1).", 20";

//$sqlaa = "SELECT  DB_data, DB_title from F007 WHERE DB_title like '%[".$dream_name."]%' order by DB_num desc limit 0, 10";
...
<?
	...
	for ($i = 0 ; $i<$all; $i++) {
		mysql_data_seek($query,$i);
		$row = mysql_fetch_array($query);
		$DB_title = stripslashes($row[DB_title_1]);
...
?>
	...
	<TABLE align="center">
		...
		<TR>
			...
			for($i = $startpage; $i <  $startpage + $limit -1; $i++){
				if($i == $page){
			?>
				<B><?=$i;?></B>&nbsp;
			<?}else{?>
